...
function FordFulkerson:hasPath()
    ...
    local queue = dofile(currentDirectory .. '\\YAYA\\Module\\Graph\\data\\queue.lua').create()
...
function FordFulkerson:minCuts()
    local result = dofile(currentDirectory .. '\\YAYA\\Module\\Graph\\data\\list.lua').create()
    for i = 0,self.network:vertexCount()-1 do
        local v = self.network:vertexAt(i)
