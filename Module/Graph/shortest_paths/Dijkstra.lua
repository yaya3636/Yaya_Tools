...
function Dijkstra:run(G, s)
    ...
    local pq = dofile(currentDirectory .. '\\YAYA\\Module\\Graph\\data\\IndexedMinPQ.lua').create()
    self.cost[s] = 0
...
function Dijkstra:getPathTo(v)
    local stack = dofile(currentDirectory .. '\\YAYA\\Module\\Graph\\data\\stack.lua').create()
    local x = v
